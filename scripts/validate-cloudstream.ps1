...
if ($SkipBuild) {
    # Mevcut artifact ile yalnızca ağ/metadata kontrolleri isteniyor.
    $expectedVersion = [int]$generatedEntry.version
}
...
if (-not $SkipNetwork) {
    ...
    Write-Host "[5b/6] NOW TV sayfasından taze imzalı HLS adresi test ediliyor..."
    $nowHeaders = @{
        "User-Agent" = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/153.0.0.0 Safari/537.36"
        "Accept-Language" = "tr-TR,tr;q=0.9,en-US;q=0.8,en;q=0.7"
    }
    $nowPage = Invoke-WebRequest "https://www.nowtv.com.tr/canli-yayin" -Headers $nowHeaders -UseBasicParsing
    $nowUrl = [regex]::Matches($nowPage.Content, 'https?://[^"''\s]+\.m3u8[^"''\s]*') |
        ForEach-Object { $_.Value.Replace("&amp;", "&").Replace("\u0026", "&") } |
        Where-Object { $_ -like "*nowtv.daioncdn.net/nowtv/nowtv.m3u8*" } |
        Select-Object -First 1
    Assert-Condition (-not [string]::IsNullOrWhiteSpace($nowUrl)) "NOW TV sayfasından master HLS adresi alınamadı."
    Assert-Condition ($nowUrl -match '[?&]st=' -and $nowUrl -match '[?&]e=') "NOW TV HLS adresinde geçici imza parametreleri yok."
    $nowPlaylistResponse = Invoke-WebRequest $nowUrl -Headers ($nowHeaders + @{
        "Origin" = "https://www.nowtv.com.tr"
        "Referer" = "https://www.nowtv.com.tr/"
    }) -UseBasicParsing
    $nowPlaylistText = if ($nowPlaylistResponse.Content -is [byte[]]) {
        [Text.Encoding]::UTF8.GetString($nowPlaylistResponse.Content)
    } else {
        $nowPlaylistResponse.Content
    }
    Assert-Condition ($nowPlaylistResponse.StatusCode -eq 200 -and $nowPlaylistText -match '#EXTM3U') "NOW TV HLS playlisti alınamadı."
    Write-Host "  OK NOW TV"
} else {
...
if ($CheckRemote) {
    ...
    Assert-Condition ([int]$remoteMeta.version -eq $expectedVersion) "GitHub'daki plugin sürümü build.gradle.kts ile eşleşmiyor."
} else {
